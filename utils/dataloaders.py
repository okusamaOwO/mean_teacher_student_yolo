...
class LoadImagesAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Detach depth before Albumentations/HSV since they expect 3 channels
        ...
        if im is None:  # not cached in RAM
            ...
            if self.depth_dir is not None:
                ...
                depth_path = os.path.join(
                    self.depth_dir, split_dir, depth_name)
                ...
                if os.path.isfile(depth_path):
                    ...
                    if depth.shape != im.shape[:2]:
                        LOGGER.warning(
                            f'WARNING ⚠️ Depth shape {depth.shape} does not match image shape {im.shape[:2]} for {f}. Skipping depth.')
                    else:
                        # Add depth as a fourth channel
                        im = np.dstack((im, depth))
                else:
                    print(
                        f'WARNING ⚠️ Depth file {depth_path} not found for image {f}. Skipping depth.')
            h0, w0 = im.shape[:2]  # orig hw
            r = self.img_size / max(h0, w0)  # ratio
            if r != 1:  # if sizes are not equal
                interp = cv2.INTER_LINEAR if (
                    self.augment or r > 1) else cv2.INTER_AREA
                # Added im.shape[2] == 4 check!
                if self.depth_dir is not None and im.shape[2] == 4:
                    im_rgb = im[:, :, :3]
                    im_depth = im[:, :, 3]
                    im_rgb = cv2.resize(
                        im_rgb, (int(w0 * r), int(h0 * r)), interpolation=interp)
                    im_depth = cv2.resize(
                        im_depth, (int(w0 * r), int(h0 * r)), interpolation=cv2.INTER_NEAREST)
    ...
    def collate_fn(batch):
        im, label, path, shapes, depths = zip(
            *batch)  # transposed (5 items now)
        for i, lb in enumerate(label):
            lb[:, 0] = i  # add target image index for build_targets()
        return torch.stack(im, 0), torch.cat(label, 0), path, shapes, torch.stack(depths, 0)
...
# Ancillary functions --------------------------------------------------------------------------------------------------
def flatten_recursive(path=DATASETS_DIR / 'coco'):
...
def extract_boxes(path=DATASETS_DIR / 'coco'):
    # Convert detection dataset into classification dataset, with one directory per class
    path = Path(path)  # images dir
    shutil.rmtree(path / 'classification') if (path /
                                               # remove existing
                                               'classification').is_dir() else None
...
def autosplit(path=DATASETS_DIR / 'coco/images', weights=(0.9, 0.1, 0.0), annotated_only=False):
...
class HUBDatasetStats():
    ...
    Usage
        ...
        stats = HUBDatasetStats('coco.yaml', autodownload=True)  # usage 1
        stats = HUBDatasetStats('path/to/coco.zip')  # usage 2
        stats.get_json(save=False)
        stats.process_images()
    """

    def __init__(self, path='coco.yaml', autodownload=False):
        # Initialize class
