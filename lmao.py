...
def save_mixed_images(mixed_source_imgs, mixed_target_imgs, save_dir, epoch, batch_idx):
    ...
    LOGGER.info(
        f'Saved {len(mixed_source_imgs)} source and {len(mixed_target_imgs)} target images to {epoch_dir}')
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    student_optimizer = smart_optimizer(
    ...
    def get_activation(name):
        ...
        return hook
    hook_handle = de_parallel(student_model).model[0].register_forward_hook(
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (source_imgs, source_labels, paths,
                _) in pbar:  # batch -------------------------------------------------------------
            target_imgs, _, target_paths, _ = next(target_iter)  # PURE FOG
            ...
            source_imgs = source_imgs.to(
                device, non_blocking=True).float() / 255  # PURE CLEAR
            target_imgs = target_imgs.to(
                device, non_blocking=True).float() / 255  # PURE FOGGY

            X1 = torch.cat((source_imgs, target_imgs), 0)
            X2 = torch.cat((target_imgs, source_imgs), 0)
            B_total = X1.size(0)
            B_source = source_imgs.size(0)
            B_target = imgs_student.size(0)
            mixed_batch = mixstyle_cross_domain(X1, X2, alpha=0.3)
            mixed_source_imgs = mixed_batch[:B_source]      # MIXED CLEAR
            mixed_target_imgs = mixed_batch[B_source:]      # MIXED FOGGY

            # hmm, đây đang là unsupervised learning, thì làm sao có mấy cái nhãn để tính loss bình thường được nhỉ

            # student_imgs = torch.cat((imgs_student, mixed_target_imgs), 0)  # [original_target, mixed_target]
            ...
            # [original_source, mixed_source]
            source_imgs = torch.cat((source_imgs, mixed_source_imgs), 0)
            # Duplicate labels for both original and mixed
            source_labels = torch.cat((source_labels, source_labels), 0)
            ...
            if opt.multi_scale:
                ...
                if sf != 1:
                    ...
                    imgs_student = nn.functional.interpolate(imgs_student, size=ns, mode='bilinear',
                                                             align_corners=False)
                    student_imgs = nn.functional.interpolate(student_imgs, size=ns, mode='bilinear',
                                                             align_corners=False)
            ...
            # Log
            if RANK in {-1, 0}:
                mloss = (mloss * i + combined_loss_items) / \
                ...
                callbacks.run('on_train_batch_end', student_model,
                              ni, source_imgs, source_labels, paths, list(mloss))
