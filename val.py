...
def run(
    ...
    for batch_i, batch in enumerate(pbar):
        if len(batch) == 4:
            ...
            im, targets, paths, shapes, depth_maps = batch
        ...
        if depth_maps is not None:
            ...
            # prevent division by zero in val too
            depth_maps = torch.nan_to_num(
                depth_maps, nan=0.0, posinf=0.0, neginf=0.0)
            max_d = torch.max(depth_maps)
            if max_d > 0:
                depth_maps = depth_maps / max_d
        ...
        with dt[1]:
            if depth_maps is not None:
                preds, train_out = model(im, depth_maps) if compute_loss else (
                    model(im, depth_maps, augment=augment), None)
            else:
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str,
                        default=ROOT / 'yolo.pt', help='model path(s)')
    ...
    parser.add_argument('--iou-thres', type=float,
                        default=0.7, help='NMS IoU threshold')
