...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        # --- CUSTOM LAYER SHIFT FOR CBAM HEAD ---
        # assuming you run this with your modified yolov9-s.yaml
        if 'yolov9-s-cbam' in str(cfg) or 'yolov9-s.yaml' in str(cfg):
            mapped_csd = {}
            import re
            for k, v in csd.items():
                ...
                if old_idx <= 14:
                    new_idx = old_idx  # Match exactly up to the Concat before first CBAM
                elif 15 <= old_idx <= 27:
                    new_idx = old_idx + 1  # Shift down 1 for the first inserted CBAM
                elif old_idx == 28:
                    new_idx = old_idx + 2  # Shift down 2 for the second inserted CBAM
                elif old_idx == 29:
                    continue  # Drop old DualDDetect head
                else:
                    new_idx = None
